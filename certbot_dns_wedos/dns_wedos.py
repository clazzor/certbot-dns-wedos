...
from certbot_dns_wedos import TTL, URL, WEDOS_CODE
...
class _WedosClient:
    ...
    def _handler_wedos(self, response: requests.post) -> dict:
        ...
        if data["response"]["code"] >= 2000:
            raise errors.PluginError(
                "Error code received from Wedos API, The error "
    ...
    @convert_domain
    def del_txt_record(
        self, domain: str, _validation_name: str, validation: str
    ) -> None:
...
class Authenticator(DNSAuthenticator):
    ...
    def add_parser_arguments(
        cls, add: Callable[..., None], default_propagation_seconds: int = 450
    ) -> None:
    ...
    def _validate_credentials(self, credentials: CredentialsConfiguration) -> None:
        ...
        if propagation_seconds < 300:
            raise errors.PluginError(
                "Propagation seconds cannot be lower than 300 seconds."
